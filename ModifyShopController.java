 ...
 * @author mon
 */
public class ModifyShopController implements Initializable {
   @FXML
  ...
  @FXML
    ...
    private void Enregistrer(ActionEvent event) throws IOException {
       ShopService sv = new ShopService();
        sv.modifierFeed1(x,nom.getText(),addresse.getText(),etoiles.getText(),tel.getText(),prix.getText());
        try {
            javafx.scene.Parent tableview = FXMLLoader.load(getClass().getResource("ModifyShop.fxml"));
            Scene sceneview = new Scene(tableview);
            Stage window = (Stage) ((Node) event.getSource()).getScene().getWindow();
            window.setScene(sceneview);
            window.show();
        ...
        String Etoiles = etoiles.getText();
