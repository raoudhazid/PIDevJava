 ...
 * @author mon
 */
public class ModifyRestauController implements Initializable {
   @FXML
  ...
  @FXML
    ...
    private void Enregistrer(ActionEvent event) throws IOException {
       RestaurantService sv = new RestaurantService();
        sv.modifierFeed1(x,nom.getText(),addresse.getText(),specialite.getText());
        try {
            javafx.scene.Parent tableview = FXMLLoader.load(getClass().getResource("ModifyRestau.fxml"));
            Scene sceneview = new Scene(tableview);
            Stage window = (Stage) ((Node) event.getSource()).getScene().getWindow();
            window.setScene(sceneview);
            window.show();
        ...
        String Addresse = addresse.getText();
         ...
         if ( Nom.trim().length() < 1 ||Addresse.trim().length() < 1 || Specialite.trim().length() < 1  ) {
