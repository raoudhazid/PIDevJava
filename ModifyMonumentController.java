 ...
 * @author Lenovo
 */
public class ModifyMonumentController implements Initializable{
  ...
  @FXML
     ...
     */
    @FXML
    private void Enregistrer(ActionEvent event) throws IOException, MessagingException {
       MonumentService sv = new MonumentService();
        sv.modifierFeed1(x,nom.getText(),destination.getText(),addresse.getText(),description.getText(),Integer.parseInt(prix.getText()));
        try {
            javafx.scene.Parent tableview = FXMLLoader.load(getClass().getResource("ModifyMonument.fxml"));
            Scene sceneview = new Scene(tableview);
            Stage window = (Stage) ((Node) event.getSource()).getScene().getWindow();
            window.setScene(sceneview);
            window.show();
        ...
        String Description = description.getText();
         ...
         if ( Nom.trim().length() < 1 ||Addresse.trim().length() < 1 || Description.trim().length() < 1  ) {
